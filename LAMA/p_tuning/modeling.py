...
class PTuneForLAMA(torch.nn.Module):
    ...
    def embed_input(self, queries):
        ...
        # 只finetune prompt，在这里替换更新后的prompt
    ...
    def get_query(self, x_h, prompt_tokens, x_t=None):
        ...
        if 'gpt' not in self.args.model_name and 'megatron' not in self.args.model_name:
            # BERT-style model
            # [CLS][prompt_tokens][prompt_tokens][[prompt_tokens][mask][prompt_tokens][prompt_tokens][prompt_tokens][text...][prompt_tokens][prompt_tokens][[prompt_tokens][sep]
    ...
    def forward(self, x_hs, x_ts, return_candidates=False):
        ...
        # construct query ids
        prompt_tokens = [self.pseudo_token_id] # prompt_tokens:[28996]
